...
async function extractKeysFromWeChat({ dbDir, wxDir, onProgress, loginCapture = true, keysPath = null }) {
  ...
  const { extractKeysViaDllHook, isDllHookAvailable, resolveDllPath, getLastHookPassphrase } = require('./wxKeyHook');
  ...
  if (remainingSalts.size > 0 && newMemoryModel && loginCapture) {
    if (isDllHookAvailable()) {
      log(`已找到 Hook DLL: ${resolveDllPath()}`);
      try {
        ...
        log(`Hook 失败: ${err.message}`);
      }
    } else {
      log('');
      log('未找到内置 Hook 模块 wexin_hook.dll');
      log('请运行: npm run build:hook（需要 Visual Studio C++ 构建工具）');
      log('或从 GitHub Actions 下载 wexin_hook.dll 放到 assets/dll/');
      log('');
    }
  }
  ...
  if (Object.keys(keyMap).length === 0) {
    throw new Error(
      [
        '未能获取数据库密钥。',
        '',
        '微信 4.1.10.53 已无法通过内存扫描获取密钥（hex 模式恒为 0 是正常现象）。',
        '本工具使用内置 Hook 模块拦截 Weixin.dll 的 setCipherKey，无需 ExportWeChat。',
        '',
        '解决方案（任选其一）：',
        '1. 运行 npm run build:hook 构建 wexin_hook.dll，以管理员身份重试',
        '2. 将 all_keys.json 放到微信账号目录，或在工具中选择「密钥文件」导入',
      ].join('\n')
    );
  }
