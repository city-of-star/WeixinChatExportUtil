  ...
  if (Object.keys(keyMap).length === 0) {
    ...
    if (newMemoryModel) {
      if (!isDllHookAvailable()) {
        ...
        lines.push(
          '微信 4.1.10+ 需要登录时捕获密钥。',
          '请在「高级选项」中勾选「登录时自动获取解密密钥」后重试。'
        );
      } else if (hookFailed || hookAttempted) {
        lines.push(
          '自动获取密钥未成功，请按以下步骤重试：',
          '1. 右键本程序，选择「以管理员身份运行」',
          '2. 扫描时若微信被重新打开，请在微信窗口点击「登录」',
          '3. 等待提示「Hook 已就绪」后再点击登录',
          '4. 或在「高级选项」中手动指定密钥文件'
        );
      } else {
        lines.push(
