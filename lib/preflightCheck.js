...
function runPreflightChecks({ wxDir, accountPath = null, readiness: readinessInput = null }) {
  ...
  if (resolvedAccount) {
    try {
      ...
      hasDecrypted = hasDecryptedStorage(resolvedAccount);
  ...
  if (isAdmin) {
    checks.push(
      makeCheck('admin', 'pass', '已以管理员身份运行', 'Hook 注入与内存读取权限正常')
    );
  } else if (hasDecrypted) {
    checks.push(
      makeCheck(
        ...
        'pass',
        '未以管理员身份运行',
        '已有本地解密数据，通常可直接扫描；若需重新解密请以管理员身份运行'
      )
    );
  } else {
    checks.push(
      makeCheck(
        ...
        '未以管理员身份运行',
        newMemoryModel
          ? '微信 4.1.10+ 首次解密需要 Hook，建议右键 exe 选择「以管理员身份运行」；也可先尝试继续扫描'
          : '读取微信进程内存可能需要管理员权限；若扫描失败请以管理员身份重试'
      )
    );
  }
