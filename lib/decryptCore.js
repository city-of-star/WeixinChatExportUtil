...
function hasDecryptedStorage(wxDir) {
  return resolveDecryptedDir(wxDir) != null;
}
...
function readDecryptInfo(wxDir) {
  const decryptedDir = resolveDecryptedDir(wxDir);
  if (!decryptedDir) {
    return null;
  }
...
async function ensureDecrypted({ wxDir, forceDecrypt = false, loginCapture = true, keysPath = null, onProgress }) {
  if (!needsDecrypt(wxDir, forceDecrypt)) {
    ...
    if (decryptedDir) {
      ...
      return { skipped: true, decryptedDir };
    }
  ...
  try {
    ...
    const fallbackDir = resolveDecryptedDir(wxDir);
