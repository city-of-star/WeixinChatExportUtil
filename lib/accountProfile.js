...
function readSelfNickFromContactDb(SQL, decryptedDir, wxid) {
  ...
  try {
    ...
    const rows = queryAll(
      db,
      `SELECT * FROM contact WHERE username = '${safeWxid}' OR alias = '${safeWxid}' LIMIT 1`
    );
    const row = rows[0];
    return pickField(row, ['remark', 'nick_name', 'nickname', 'NickName', 'Remark']) || null;
  } catch {
    return null;
  } finally {
    db.close();
...
function readAvatarFromHeadImageDb(SQL, decryptedDir, wxid) {
  ...
  try {
    ...
    for (const { name } of tables) {
      ...
      const rows = queryAll(
        db,
        `SELECT "${blobCol}" AS blob_data FROM "${name}" WHERE "${userCol}" = '${safeWxid}' LIMIT 1`
      );
...
async function enrichAccountSummary(summary) {
  const { wxid, path: accountDir } = summary;
  ...
  if (decryptedDir) {
    try {
      ...
      displayName = readSelfNickFromContactDb(SQL, decryptedDir, wxid) || displayName;
