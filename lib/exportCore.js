  ...
  try {
    for (const { name: table } of listMsgTables(db)) {
      ...
      const result = db.exec(
        `SELECT count(*) AS c, sum(CASE WHEN local_type = 34 THEN 1 ELSE 0 END) AS v FROM "${table}"`
      );
      ...
      counts[username] = [count, Number(values[1]) || 0];
    }
  } finally {
    db.close();
  }
...
function mergeDbStatsToCounts(dbStats) {
  ...
  return { messageCounts, voiceCounts };
}
...
function buildMessageAndVoiceCounts(msgDbs, SQL, usernames, onProgress) {
  ...
  const { messageCounts, voiceCounts } = mergeDbStatsToCounts(dbStats);
  return { messageCounts, voiceCounts, dbStats };
}
...
function buildMessageAndVoiceCountsIncremental(msgDbs, SQL, usernames, previousDbStats, onProgress) {
  ...
  for (let dbIndex = 0; dbIndex < msgDbs.length; dbIndex += 1) {
    ...
    if (cached?.fingerprint === fingerprint && cached?.counts) {
      dbStats[key] = cached;
      reused += 1;
    } else {
      dbStats[key] = {
  ...
  const { messageCounts, voiceCounts } = mergeDbStatsToCounts(dbStats);
  return { messageCounts, voiceCounts, dbStats, reused, rescanned: totalDbs - reused };
}
...
async function exportWeChatChats({
  ...
  } else if (!hasDecryptedStorage(resolvedWxDir)) {
    throw new Error(
      '未找到已解密数据库。请先在第二步「扫描会话」，或选择历史扫描记录；若需同步微信最新数据，请使用「重新扫描」。'
    );
  } else {
    onProgress?.({
      phase: 'init',
